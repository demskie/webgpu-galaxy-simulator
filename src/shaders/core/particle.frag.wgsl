...
fn main(input: FragmentInput) -> FragmentOutput {
    ...
    // Check overdraw limit using atomic counter
    ...
    // Atomic increment and check limit (unless in overdraw debug mode)
    if (uniforms.galaxy.overdrawDebug < 0.5) {
        let currentCount = atomicAdd(&overdrawCounts[pixelIndex], 1u);
        
        // Compute effective per-frame limit scaled by temporal accumulation
        // This makes overdraw limiting consistent across accumulation levels
        ...
        // In debug mode, still count but don't limit
        atomicAdd(&overdrawCounts[pixelIndex], 1u);
    }
    ...
    if (input.typ == 0.0) { // STAR type
        ...
        // This helps reduce flickering by providing more intermediate alpha values
        ...
        // Add a subtle glow to bright stars to help with anti-aliasing
        if (dist > softness) {
